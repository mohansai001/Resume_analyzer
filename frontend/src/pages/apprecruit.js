...
function AppRecruit() {
  ...
  const [selectedTile, setSelectedTile] = useState('App Engineering');
  ...
  const extractJdInfo = (jdContent) => {
    const title = jdContent.match(/(?:position|role|job title|title)\s*:?\s*([^\n\r]+)/i)?.[1]?.trim() || 
                  jdContent.split('\n')[0]?.trim().substring(0, 100) || 'Not specified';
    ...
    try {
      ...
      sessionStorage.setItem(
        ...
        JSON.stringify({
          ...
          ecName: selectedTile || 'App Engineering',
        })
      );
  ...
  if (isLoading) {
    return (
      <div id="loading-popup" style={{ display: 'flex' }}>
        ...
        ></div>
        <span style={{ marginTop: '20px' }}>Please wait while resume(s) is being evaluated!...</span>
      </div>
    );
  }
        ...
        <div className="role-selection-container">
          <div className="role-selection-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 20px' }}>
            ...
            <div style={{ backgroundColor: '#10b981', color: '#ffffff', padding: '8px 16px', borderRadius: '20px', fontSize: '14px', fontWeight: '600' }}>
             ...
             <div className="engineering-tiles">
        <div className={`engineering-tile ${selectedTile === 'App Engineering' ? 'selected' : ''}`} onClick={() => handleTileClick('App Engineering')}>
          <span className="tile-text">App Engineering</span>
        </div>
        <div className={`engineering-tile ${selectedTile === 'Cloud Engineering' ? 'selected' : ''}`} onClick={() => handleTileClick('Cloud Engineering')}>
          <span className="tile-text">Cloud Engineering</span>
        </div>
        <div className={`engineering-tile ${selectedTile === 'Core Engineering' ? 'selected' : ''}`} onClick={() => handleTileClick('Core Engineering')}>
          <span className="tile-text">Core Engineering</span>
        </div>
        <div className={`engineering-tile ${selectedTile === 'Data Engineering' ? 'selected' : ''}`} onClick={() => handleTileClick('Data Engineering')}>
          <span className="tile-text">Data Engineering</span>
        </div>
        <div className={`engineering-tile ${selectedTile === 'Quality Engineering' ? 'selected' : ''}`} onClick={() => handleTileClick('Quality Engineering')}>
          <span className="tile-text">Quality Engineering</span>
        </div>
        <div className={`engineering-tile ${selectedTile === 'Cross Functionality' ? 'selected' : ''}`} onClick={() => handleTileClick('Cross Functionality')}>
          <span className="tile-text">Cross Functionality</span>
        </div>
      </div>


          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '30px', padding: '20px', backgroundColor: '#000000', minHeight: '100vh' }}>
            <form onSubmit={handleUploadResume} style={{ width: '600px', padding: '40px', backgroundColor: '#1a1a1a', borderRadius: '15px', boxShadow: '0 10px 30px rgba(16, 185, 129, 0.2)', border: '1px solid #10b981', marginRight: '20px' }}>
              ...
              {/* Selected Engineering Tile Display */}
              {selectedTile && (
                <div className="selected-engineering-tile" style={{ marginBottom: '25px' }}>
                  <span className="selected-tile-text">{selectedTile}</span>
                  <button className="close-selected" onClick={closeTileSelection}>×</button>
                </div>
              )}
              ...
              <div style={{ marginBottom: '25px' }}>
                <label htmlFor="jobDescription" style={{ fontColor: 'white !important', display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: '500' }}>📄 Job Description</label>
                <div style={{ border: '2px dashed #10b981', borderRadius: '8px', padding: '20px', textAlign: 'center', backgroundColor: '#000000', transition: 'all 0.3s ease' }}>
                  ...
                  <label htmlFor="jobDescription" className="upload-instruction-text" style={{ cursor: 'pointer', fontSize: '16px', fontWeight: '500' }}>📁 Click to upload job description</label>
              ...
              <div style={{ marginBottom: '30px' }}>
                <label htmlFor="resume" style={{ fontColor: 'white !important', display: 'block', marginBottom: '8px', fontSize: '14px', fontWeight: '500' }}>📎 Resume Files</label>
                <div style={{ border: '2px dashed #10b981', borderRadius: '8px', padding: '20px', textAlign: 'center', backgroundColor: '#000000', transition: 'all 0.3s ease' }}>
                  ...
                  <label htmlFor="resume" className="upload-instruction-text" style={{ cursor: 'pointer', fontSize: '16px', fontWeight: '500' }}>📁 Click to upload resume files</label>
...
const EvaluationResultView = ({ result, onBack, selectedRoleInfo, sendCandidateInfoToDB }) => {
  ...
  useEffect(() => {
    ...
    // Extract role from job description stored in sessionStorage
    const sessionData = JSON.parse(sessionStorage.getItem('uploadedResumes') || '{}');
    const extractedRole = sessionData.jobDescription ? extractRoleFromJD(sessionData.jobDescription) : 'General Position';
...
const extractRoleFromJD = (jobDescription) => {
  ...
  // Enhanced patterns to extract complete role/position from JD
  const rolePatterns = [
    /(?:job\s+title|position|role|title)\s*:?\s*([^\n\r]+)/i,
    /(?:we are looking for|seeking|hiring)\s+(?:a|an)?\s*([^\n\r]+?)(?:\s+to\s+|\s+who\s+|\s+with\s+|\.|\n|\r|$)/i,
    /^\s*([^\n\r]+?)(?:\s+position|\s+role|\s+job|\s+opening)\s*$/im,
    /position\s+title\s*:?\s*([^\n\r]+?)(?:\n|\r|$)/i,
    /job\s+description\s*:?\s*([^\n\r]+?)(?:\n|\r|$)/i,
    /^\s*([A-Z][^\n\r]*(?:developer|engineer|analyst|manager|specialist|consultant|architect|lead|senior|junior|associate).*?)(?:\n|\r|$)/im,
    /^\s*([^\n\r]*(?:developer|engineer|analyst|manager|specialist|consultant|architect|lead|senior|junior|associate)[^\n\r]*)(?:\n|\r|$)/im
  ];
  ...
  for (const pattern of rolePatterns) {
    ...
    if (match && match[1]) {
      let role = match[1].trim().replace(/[*#]+/g, '').replace(/\s+/g, ' ');
      // Remove common prefixes/suffixes that aren't part of the role
      role = role.replace(/^(for|a|an|the)\s+/i, '');
      role = role.replace(/\s+(at|in|with|for)\s+.*$/i, '');
      if (role.length > 3 && role.length < 150) {
        return role;
      }
    }
  }
  
  // Fallback: extract first meaningful line that contains job-related keywords
  const lines = jobDescription.split('\n').filter(line => line.trim().length > 0);
  for (const line of lines.slice(0, 5)) { // Check first 5 lines
    const cleanLine = line.trim().replace(/[*#-]+/g, '').replace(/\s+/g, ' ');
    if (cleanLine.length > 5 && cleanLine.length < 150 && 
        /(?:developer|engineer|analyst|manager|specialist|consultant|architect|lead|senior|junior|associate|position|role)/i.test(cleanLine)) {
      return cleanLine;
    }
  }
  
  // Final fallback: use first non-empty line if reasonable length
  if (lines.length > 0) {
    const firstLine = lines[0].trim().replace(/[*#-]+/g, '').replace(/\s+/g, ' ');
