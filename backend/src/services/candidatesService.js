    ...
    try {
      const query = `
        SELECT 
          ...
          END AS week,
          CASE 
            WHEN EXTRACT(DAY FROM date) <= 7 THEN DATE_TRUNC('month', CURRENT_DATE)
            WHEN EXTRACT(DAY FROM date) <= 14 THEN DATE_TRUNC('month', CURRENT_DATE) + INTERVAL '7 days'
            WHEN EXTRACT(DAY FROM date) <= 21 THEN DATE_TRUNC('month', CURRENT_DATE) + INTERVAL '14 days'
            ELSE DATE_TRUNC('month', CURRENT_DATE) + INTERVAL '21 days'
          END AS start_date,
          COUNT(*) as uploaded,
